...
func (r Running) TrainingInfo() InfoMessage {
	// вставьте ваш код ниже
	// !!! Вот тут не совсем понятно, у нас есть метод TrainingInfo() для Training
	// 1. можно вызвать его и потом присвоить полю Calories новое значение
	// 2. полностью переопределить возващаемое значение

	// 1. реализация
	trainingInfo := r.Training.TrainingInfo()
	trainingInfo.Calories = r.Calories()
	return trainingInfo
...
func (w Walking) Calories() float64 {
	// вставьте ваш код ниже
	meenSpeedInMinS := w.meanSpeed() * KmHInMsec
	heightInM := w.Height / CmInM
	durationInMuinutes := w.Duration.Hours() * MinInHours
	return ((CaloriesWeightMultiplier*w.Weight + (math.Pow(meenSpeedInMinS, 2)/heightInM)*CaloriesSpeedHeightMultiplier*w.Weight) * durationInMuinutes)
}
...
func (w Walking) TrainingInfo() InfoMessage {
	...
	trainingInfo.Calories = w.Calories()
	return trainingInfo
}
...
// distance возвращает дистанцию, которую преодолел пользователь.
// Формула расчета:
// длина бассейна * количество пересечений бассейна / м_в_км
